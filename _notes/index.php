...
PageName
<?php
function build($array, $parentId=0) {
	$needle = "e";
	$replacement = "3";
    ...
    foreach($array as $arrayItem){
        ...
        if($arrayItem['parentId'] == $parentId){
        	...
        	//Make lowercase then check 'for an 'r' 
		if(strpos(strtolower($arrayItem['pageName']), 'r') === false) {
            $tree .= '<li> 
                id= '.$arrayItem['id'].' 
                parent id='.$arrayItem['parentId'].' 
                pageName= '.str_replace($needle,$replacement,$arrayItem['pageName']).' 
                position= '.$arrayItem['position'];
                $tree .= build($array, $arrayItem['id']);
            $tree .= '</li>';
        }
    	}
